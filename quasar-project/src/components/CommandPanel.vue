  ...
  try {
    switch (command) {
      ...
      case '/join': {
        const name = args[0]
        const type = args[1]
        if (!name) { log('Použitie: /join <názov> [private]', 'error'); return }
      ...
      case '/invite': {
        if (!props.currentChannel) { log('Musíš byť v kanáli.', 'error'); return }
        const nick = args[0]
        if (!nick) { log('Použitie: /invite <nick>', 'error'); return }
      ...
      case '/revoke': {
        if (!props.currentChannel) { log('Musíš byť v kanáli.', 'error'); return }
        const nick = args[0]
        if (!nick) { log('Použitie: /revoke <nick>', 'error'); return }
      ...
      case '/kick': {
        if (!props.currentChannel) { log('Musíš byť v kanáli.', 'error'); return }
        const nick = args[0]
        if (!nick) { log('Použitie: /kick <nick>', 'error'); return }
